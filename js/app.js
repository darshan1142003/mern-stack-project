let button = document.getElementById("add-task");
 ...
 loadData();
 let Completebuttons = document.querySelectorAll(".complele-task");
 ...
 function completeTask(e) {
    e.parentElement.parentElement.querySelector('ataskHu').classList.add("completed")
    e.disabled = true;
 }

 function deleteTask(e) {
    if(element === e.parentElement.parentElement.querySelector('ataskHu').innerText)
    tasks.forEach(element => {
      tasks.splice(tasks.indexof(element),
    });
    localStorage.setItem('allTasks', JSON.stringify(tasks))
